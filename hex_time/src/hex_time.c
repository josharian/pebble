/*
   ...
   A digital watch with large hex digits.
...
static void load_digit_image_into_slot(int slot_number, int digit_value) {
  ...
  if ((digit_value < 0) || (digit_value > NUMBER_OF_IMAGES-1)) {
    return;
  }
...
static void display_value(unsigned short value, unsigned short row_number, bool show_first_leading_zero) {
  /*
     ...
     Displays a numeric value between 0 and 255 on screen.
   ...
   */
  value = value % (NUMBER_OF_IMAGES*NUMBER_OF_IMAGES - 1); // Maximum of two digits per row.
  ...
  for (int column_number = 1; column_number >= 0; column_number--) {
    ...
    if (!((value == 0) && (column_number == 0) && !show_first_leading_zero)) {
      load_digit_image_into_slot(slot_number, value % NUMBER_OF_IMAGES);
    }
    value = value / NUMBER_OF_IMAGES;
  }
}
